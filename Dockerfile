...
USER root
# Шаг 1: Устанавливаем базовые утилиты
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
    curl gnupg unzip jq wget

# Шаг 2: Добавляем ключ и репозиторий Google Chrome
RUN curl -sS -o - https://dl-ssl.google.com/linux/linux_signing_key.pub | apt-key add -
RUN echo "deb [arch=amd64] http://dl.google.com/linux/chrome/deb/ stable main" > /etc/apt/sources.list.d/google-chrome.list

# Шаг 3: Устанавливаем сам Chrome
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
    google-chrome-stable

# Шаг 4: Устанавливаем chromedriver (используем wget для большей надежности)
...
# Шаг 5: Устанавливаем зависимости для Headless и очищаем кэш
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
